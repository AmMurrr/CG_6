...
int main(int argc, char** argv)
{
    ...
    SDL_Window* window = SDL_CreateWindow("Game Engine", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1280, 720, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
    if(!window)
    {
    ...
    if(!main_context)
    {
        std::cerr << "Failed to initialize context" << SDL_GetError() << std::endl;
        return -1;
    }
    ...
    if( err != GLEW_OK)
    {
        std::cerr << "Glew NOT ok"<< glewGetErrorString(err) << std::endl;
        return -1;
    }
